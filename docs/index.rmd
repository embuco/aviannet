...
### Teach the Machine
_Our primary goal is to develop a workflow for photo tagging to support aerial digital survey efforts and support AI/ML research efforts.Fcusing on an app/interface for humans to generate training sets of images for the AI/ML process._

#### For the non-hackers:
AI/ML techniques require teaching the machine what we see.  It is truly an education by example.  We feed it lots of images and indicate which one contain targets (birds in this case) and which ones don't.  Through the magic of math and computer stuff, the AI learns which pictures contain targets and which don't. Our first goal is to construct a method for human viewers to annotate images as containing or not containing birds. This information will then be stored and given to ~~SkyNet~~ AvianNet to learn which images we humans would like to see and which to discard. 
...
_The stretch and forward looking goal is to set up a functioning process to filter images based on the training set created from the first goal._

#### For the non-hacker
We want to be lazy and have the machines do our work for us.  

#### For the hackers
We are hoping to have access to some AI / ML resources to play with existing technologies.  Possible resources include (to be filled in as we are successful at borrowing/begging)
